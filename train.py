...
if __name__ == '__main__':
    ...
    dataset_size = len(dataset)    # get the number of images / videos in the dataset. do not think dropout.
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_epoch_freq>
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            '''loss'''
            if total_iters % opt.print_freq == 0 or (total_iters <= 5000 and total_iters % (10 * opt.batch_size) ==0):  # print training losses and save logging information to the disk
        ...
        if epoch % opt.save_epoch_freq == 0:              # cache our model every <save_epoch_freq> epochs
            print('-----------------------------------------------------------------')
            print('saving the model at the end of epoch: %d,  now total iters: %d' % (epoch, total_iters + visualizer.dataset_size * (opt.epoch_count - 1)))
            print('-----------------------------------------------------------------')
