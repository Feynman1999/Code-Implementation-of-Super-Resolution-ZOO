...
Example:
    ...
    --display_ncols -1 because visualized images' size are not all the same (for visdom lib, will raise error)
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            if total_iters % opt.display_freq == 0:   # display images on visdom and save images to a HTML file
                save_result_flag = total_iters % opt.update_html_freq == 0
                model.compute_visuals()  # Calculate additional output images for visualization if you need
                visualizer.display_current_results(model.get_current_visuals(), epoch, save_result_flag)
